...
void MovePointLight::Update()
{
	...
	if (Input.GetKey(KEY_NUMPAD4))
		translation.x -= Time.GetDeltaTime();

	if (Input.GetKey(KEY_NUMPAD6))
		translation.x += Time.GetDeltaTime();

	if (Input.GetKey(KEY_NUMPAD8))
		translation.z += Time.GetDeltaTime();

	if (Input.GetKey(KEY_NUMPAD2))
		translation.z -= Time.GetDeltaTime();

	if (Input.GetKey(KEY_NUMPAD7))
