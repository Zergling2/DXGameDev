...
namespace ze
{
	...
	public:
		...
		const StaticMesh* GetMeshPtr() const { return m_spMesh.get(); }
		std::shared_ptr<StaticMesh> GetMesh() const { return m_spMesh; }
		void SetMesh(std::shared_ptr<StaticMesh> spMesh);
	...
	private:
		std::shared_ptr<StaticMesh> m_spMesh;
