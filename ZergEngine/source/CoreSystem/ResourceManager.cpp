...
#include <ZergEngine\Common\DirectXTex\DirectXTex.h>
...
std::vector<std::shared_ptr<Mesh>> ResourceManagerImpl::CreateMesh(PCWSTR path)
{
	...
	do
	{
		...
		errno_t e;
		e = _wfopen_s(&meshFile, filePath, L"rt, ccs=UTF-8");
		if (e != 0)
		...
		PCWSTR token;
		while (fgetws(line, _countof(line), meshFile))
		{
...
std::shared_ptr<Texture2D> ResourceManagerImpl::CreateTexture(PCWSTR path)
{
	...
	texture->m_cpTex2D = std::move(cpTex2d);
	texture->m_cpSRV = std::move(cpSRV);
...
std::shared_ptr<Skybox> ResourceManagerImpl::CreateSkybox(PCWSTR path)
{
	...
	if (wcscmp(L".dds", ext) != 0 && wcscmp(L".DDS", ext) != 0)
		Debug::ForceCrashWithMessageBox(L"Unsupported Format", L"%s is not supported format for skybox.", path);

	hr = LoadFromDDSFile(filePath, DDS_FLAGS::DDS_FLAGS_NONE, nullptr, mipChain);	// dds, DDS
	if (FAILED(hr))
		Debug::ForceCrashWithHRESULTErrorMessageBox(L"CreateSkybox() > LoadFromDDSFile()", hr);
	...
	// 텍스처 생성
	ComPtr<ID3D11Texture2D> cpTex2dSkyboxCubemap;
	{
		/*
	...
	// 셰이더 리소스 뷰 생성
	ComPtr<ID3D11ShaderResourceView> cpSRVSkybox;
	D3D11_SHADER_RESOURCE_VIEW_DESC descSRV;
	ZeroMemory(&descSRV, sizeof(descSRV));
	descSRV.Format = descTexture.Format;
	...
	skybox->m_cpTex2D = std::move(cpTex2dSkyboxCubemap);
	skybox->m_cpSRV = std::move(cpSRVSkybox);
			...
			else if (!wcscmp(token, L"map_Kd") && importTexture)
			{
				if (currentMaterial)
				{
					...
					*texPathInitPtr = L'\0';
					...
					currentMaterial->m_diffuseTexture = Resource::CreateTexture(texPath);
				}
			}
			else
			{
...
std::shared_ptr<ShaderResourceTexture> Resource::LoadTexture_deprecated(const wchar_t* path)
{
	...
	{
		AsyncLogBuffer* pLogBuffer = AsyncFileLogger::GetLogBuffer();
		StringCbPrintfW(pLogBuffer->buffer, AsyncLogBuffer::BUFFER_SIZE, L"[Resource::CreateTexture] Invalid file extension. %s\n", path);
		AsyncFileLogger::Write(pLogBuffer);
