...
void GraphicDevice::CreateSamplerStates()
{
	...
	samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;		// Æ÷ÀÎÆ® »ùÇÃ¸µ
	...
	ZeroMemory(&samplerDesc, sizeof(samplerDesc));
	samplerDesc.Filter = D3D11_FILTER_MIN_MAG_LINEAR_MIP_POINT; // ½Ö¼±Çü (Bilinear)
	samplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.ComparisonFunc = D3D11_COMPARISON_ALWAYS;
	...
	ZeroMemory(&samplerDesc, sizeof(samplerDesc));
	samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR; // »ï¼±Çü (Trilinear)
	samplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.ComparisonFunc = D3D11_COMPARISON_ALWAYS;
...
void GraphicDevice::CreateDepthStencilStates()
{
	...
	// 1. Default
	depthStencilDesc.DepthEnable = TRUE;							// µª½º Å×½ºÆ® È°¼ºÈ­
	depthStencilDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;	// ±íÀÌ °ª ±â·Ï Çã¿ë
	depthStencilDesc.DepthFunc = D3D11_COMPARISON_LESS;				// ´õ °¡±î¿î ÇÈ¼¿¸¸ ±×¸®±â
	depthStencilDesc.StencilEnable = FALSE;							// ½ºÅÙ½Ç ºñÈ°¼ºÈ­
	...
	depthStencilDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;	// ½ºÄ«ÀÌ¹Ú½º ·»´õ¸µ ½Ã Áß¿ä
...
void GraphicDevice::CreateCommonVertexBuffers()
{
	...
	// 1. Button VB
	{
		const XMFLOAT2 ltShade = XMFLOAT2(+0.5f, -0.5f);			// À½¿µ Ã³¸®°ª
		const XMFLOAT2 rbShade = XMFLOAT2(ltShade.y, ltShade.x);	// À½¿µ Ã³¸®°ª
		const XMFLOAT2 centerShade = XMFLOAT2(0.0f, 0.0f);
		const float offsetValue = 2.0f;		// ¹öÆ° ÀÔÃ¼°¨ ¸ð¼­¸® ¿ÀÇÁ¼Â
		VFButton v[30];
