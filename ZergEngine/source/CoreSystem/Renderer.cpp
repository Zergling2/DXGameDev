...
void XM_CALLCONV Renderer::RenderPNTTSkinnedMesh(const SkinnedMeshRenderer* pSkinnedMeshRenderer, FXMMATRIX worldMatrix)
{
	...
	{
		XMFLOAT4X4A finalTransforms[MAX_BONE_COUNT];
		pSkinnedMeshRenderer->ComputeFinalTransform(finalTransforms, _countof(finalTransforms));

		const size_t boneCount = pArmature->GetBoneCount();

		// HLSL 전달을 위해 Mf 전치
		for (size_t boneIndex = 0; boneIndex < boneCount; ++boneIndex)
			XMStoreFloat4x4A(&finalTransforms[boneIndex], ConvertToHLSLMatrix(XMLoadFloat4x4A(&finalTransforms[boneIndex])));

		m_basicEffectPNTTSkinned.SetArmatureFinalTransform(finalTransforms, boneCount);
	}
