...
void BillboardEffect::Init()
{
	...
	m_pVS = GraphicDevice::GetInstance()->GetVSComInterface(VertexShaderType::ToHcsPNTTQuadForBillboard);
...
void XM_CALLCONV BillboardEffect::SetWorldMatrix(FXMMATRIX w) noexcept
{
	...
	m_dirtyFlag |= DirtyFlag::UpdateCBPerBillboard;
}

void BillboardEffect::SetUVScale(FLOAT sx, FLOAT sy) noexcept
{
	m_cbPerBillboardCache.uvScale.x = sx;
	m_cbPerBillboardCache.uvScale.y = sy;

	m_dirtyFlag |= DirtyFlag::UpdateCBPerBillboard;
}

void BillboardEffect::SetUVOffset(FLOAT x, FLOAT y) noexcept
{
	m_cbPerBillboardCache.uvOffset.x = x;
	m_cbPerBillboardCache.uvOffset.y = y;
...
void BillboardEffect::ApplyImpl(ID3D11DeviceContext* pDeviceContext) noexcept
{
	...
	while (BitScanForward(&index, m_dirtyFlag))
	{
		...
		case DirtyFlag::ApplyCBPerCamera:
			...
			break;
		case DirtyFlag::ApplyCBPerBillboard:
		...
		case DirtyFlag::UpdateCBPerCamera:
			...
			break;
		case DirtyFlag::UpdateCBPerBillboard:
...
void BillboardEffect::OnUnbindFromDeviceContext() noexcept
{
	ClearTextureSRVArray();
	...
	const DWORD except =
		...
		DirtyFlag::UpdateCBPerBillboard |
