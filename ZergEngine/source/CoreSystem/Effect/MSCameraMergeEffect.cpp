...
void MSCameraMergeEffect::Init()
{
	...
	m_pPixelShader = GraphicDevice.GetPSComInterface(PIXEL_SHADER_TYPE::COLOR_PT_FRAGMENT_SINGLE_MS_TEXTURE);
...
void MSCameraMergeEffect::KickedFromDeviceContext() noexcept
{
	ClearTextureSRVArray();
...
void MSCameraMergeEffect::ApplyPerMSCameraMergeConstantBuffer(ID3D11DeviceContext* pDeviceContext) noexcept
{
	...
	ID3D11Buffer* const cbArr[] = { m_cbPerMSCameraMerge.GetComInterface() };
