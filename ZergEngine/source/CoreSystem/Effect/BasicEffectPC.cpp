...
void BasicEffectPC::Init()
{
	...
	m_pInputLayout = GraphicDevice.GetILComInterface(VERTEX_FORMAT_TYPE::POSITION_COLOR);
	m_pVertexShader = GraphicDevice.GetVSComInterface(VERTEX_SHADER_TYPE::TRANSFORM_PC_TO_HCS);
...
void XM_CALLCONV BasicEffectPC::SetWorldMatrix(XMMATRIX w) noexcept
{
...
void BasicEffectPC::KickedFromDeviceContext() noexcept
{
	m_dirtyFlag = DIRTY_FLAG::ALL;
}
...
void BasicEffectPC::ApplyPerMeshConstantBuffer(ID3D11DeviceContext* pDeviceContext) noexcept
{
	...
	pDeviceContext->VSSetConstantBuffers(1, 1, cbArr);
}
