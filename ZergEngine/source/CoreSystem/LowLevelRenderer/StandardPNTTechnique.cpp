...
void StandardPNTTechnique::Apply(const D3D11_VIEWPORT* pViewport, ID3D11RenderTargetView* pRTVColorBuffer, ID3D11DepthStencilView* pDSVColorBuffer,
	RASTERIZER_FILL_MODE rfm, RASTERIZER_CULL_MODE rcm, TEXTURE_FILTERING_OPTION tfo)
{
	...
	// 인풋 레이아웃 설정
	this->GetDeviceContext()->IASetInputLayout(GraphicDevice::GetInstance().GetILComInterface(VERTEX_FORMAT_TYPE::POSITION_NORMAL_TEXCOORD));

	// 셰이더 설정
	this->GetDeviceContext()->VSSetShader(
		GraphicDevice::GetInstance().GetVSComInterface(VERTEX_SHADER_TYPE::STANDARD_TRANSFORM_PNT), nullptr, 0
	);
	this->GetDeviceContext()->PSSetShader(
		GraphicDevice::GetInstance().GetPSComInterface(PIXEL_SHADER_TYPE::STANDARD_COLORING_PNT), nullptr, 0
	);
}

void StandardPNTTechnique::Render(const MeshRenderer* pMeshRenderer)
{
	HRESULT hr;
	...
	for (const auto& subset : pMesh->m_subsets)
	{
		...
		if (pMaterial != nullptr && pMaterial->m_baseMap)
		{
			...
			this->GetDeviceContext()->PSSetShaderResources(ShaderResourceSlot::BasemapSlot::GetSlotNumber(), _countof(srvs), srvs);
		}
		/*
		else	// D3D11 ERROR 오류 메시지 방지
		{
			ID3D11ShaderResourceView* srvs[] = { nullptr };
			this->GetDeviceContext()->PSSetShaderResources(ShaderResourceSlot::BasemapSlot::GetSlotNumber(), 1, srvs);
		}
		*/
