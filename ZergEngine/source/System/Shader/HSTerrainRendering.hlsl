...
{
    const float d = distance(v, cb_camPosW);
    const float s = saturate((d - cb_tessMinDist) / (cb_tessMaxDist - cb_tessMinDist));
    
    return pow(2, lerp(cb_maxTessExponent, cb_minTessExponent, s));
}
...
DSInputQuadPatchTess
    CHSTerrainRendering(
    InputPatch<HSInputTerrainPatchControlPoint, NUM_CONTROL_POINTS> patch,
    uint patchId : SV_PrimitiveID)
{
    ...
    if (IsAabbOutsideFrustum(aabbCenter, aabbExtents, cb_frustumPlane))
    {
        // 패치를 파이프라인에서 제거
